...
import random
from variable_beam_stream import pad_to_length
...
class AttnUnit(nn.Module):
    ...
    def forward(self, enc_s_top, dec_s_top, lengths=None):
        ...
        # get rid of padding locs
        if lengths is not None:
            pad_mask = (lengths.max() - torch.arange(lengths.max()).unsqueeze(0).to(lengths.device)) > lengths.unsqueeze(1)
            pad_mask = pad_to_length(pad_mask.float(), enc_s_top.shape[1], 1, 'left', pad_value=1)
            pad_mask = pad_mask * (-1e8)
            dot = dot + pad_mask.unsqueeze(2)
...
def eval_training(opt, train_loader, encoder, decoder, attention_decoder, encoder_optimizer, decoder_optimizer, attention_decoder_optimizer, criterion, using_gpu, form_manager):
    ...
    enc_outputs = torch.zeros((enc_batch.size(0), enc_max_len, encoder.hidden_size), requires_grad=True).clone()
    if using_gpu:
    ...
    for i in range(1, 3):
        enc_s[0][i] = torch.zeros((opt.batch_size, opt.rnn_size), dtype=torch.float, requires_grad=True).clone()
        dec_s[0][i] = torch.zeros((opt.batch_size, opt.rnn_size), dtype=torch.float, requires_grad=True).clone()
        if using_gpu:
...
if __name__ == "__main__":
    ...
    main_arg_parser.add_argument('-print_every',type=int, default=200,help='how many steps/minibatches between printing out the loss')
    main_arg_parser.add_argument('-rnn_size', type=int,default=150, help='size of LSTM internal state')
    ...
    main_arg_parser.add_argument('-max_epochs',type=int, default=95,help='number of full passes through the training data')
