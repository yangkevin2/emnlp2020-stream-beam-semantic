...
def variable_beam(encoder, decoder, attention_decoder, enc_w_list, word_manager, form_manager, opt, using_gpu, checkpoint):
    iters = 0
    expansions = 0
    ap, mc = opt.ap, opt.mc
    ...
    while True:
        ...
        ap_thresholds = (log_probs[:, 0] - ap).unsqueeze(1) # batch x 1
        ...
        if ((end_indices == 1e8).long() * (log_probs > -1e8).long()).sum() == 0:
            break
        count += 1
