...
def compute_accuracy(candidate_list, reference_list): # does oracle if given multiple candidates
    ...
    for i in range(len_min):
        ...
        for j in range(len(candidate_list[i])):
            ...
            if is_all_same(candidate_list[i][j], reference_list[i]):
...
def compute_tree_accuracy(candidate_list_, reference_list_, form_manager):
    ...
    for i in range(len(candidate_list_)):
        ...
        for j in range(len(candidate_list_[i])):
            if len(candidate_list_[i][j]) > 0:
                try:
                    current_candidate_list.append(norm_tree(candidate_list_[i][j], form_manager).to_list(form_manager))
                except:
                    pass # very rare fails
        candidate_list.append(current_candidate_list)
    reference_list = []
